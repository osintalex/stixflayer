...
use crate::{
    ...
    domain_objects::vocab::{
        ...
        IdentityClass, IdentitySectors, ImplementationLanguage, IndicatorPatternType,
        IndicatorType, InfrastructureType, MalwareCapability, MalwareResult, MalwareType,
        OpinionType, Region, ReportType, ThreatActorRole, ThreatActorSophistication,
        ThreatActorType, ToolType,
    },
...
impl Stix for Grouping {
    fn stix_check(&self) -> Result<(), Error> {
        ...
        if !ContextType::iter().any(|x| x.as_ref() == stix_case(&self.context)) {
...
impl Stix for Identity {
    fn stix_check(&self) -> Result<(), Error> {
        ...
        if let Some(identity_class) = &self.identity_class {
            if IdentityClass::iter().all(|x| x.as_ref() != stix_case(&identity_class)) {
                errors.push(Error::ValidationError(format!(
                    "A identity_class should come from the STIX identity-class-ov open vocabulary. Identity identity_class '{}' is not in the vocabulary.",
                    identity_class,
                )));
            }
        }
        if let Some(sectors) = &self.sectors {
            ...
            for sector in sectors {
                if IdentitySectors::iter().all(|x| x.as_ref() != stix_case(&sector)) {
                    errors.push(Error::ValidationError(format!(
...
impl Stix for Incident {
    fn stix_check(&self) -> Result<(), Error> {
        // Incident is a valid STIX 2.1 object type per spec section 4.8.
        Ok(())
    }
}
        ...
        if let Some(secondary_motivations) = &self.secondary_motivations {
            ...
            for secondary_motivation in secondary_motivations {
                if AttackMotivation::iter()
                    .all(|x| x.as_ref() != stix_case(&secondary_motivation.as_str()))
                {
                    errors.push(Error::ValidationError(format!(
...
impl Stix for Location {
    fn stix_check(&self) -> Result<(), Error> {
        ...
        if let Some(region_str) = &self.region {
            if !Region::iter().any(|x| x.as_ref() == stix_case(&region_str)) {
                errors.push(Error::ValidationError(format!(
...
impl Stix for Malware {
    ...
    fn stix_check(&self) -> Result<(), Error> {
        ...
        if let (Some(start), Some(stop)) = (&self.first_seen, &self.last_seen) {
            add_error(
                &mut errors,
                check_timestamp_ordering(start, stop, "first_seen", "last_seen", "Malware"),
            );
        }
        if let Some(malware_types) = &self.malware_types {
            ...
            for malware_type in malware_types {
                if malware_type.chars().any(|c| c.is_uppercase())
                    || malware_type.contains('_')
                    || malware_type.contains(' ')
                {
                    errors.push(Error::ValidationError(format!(
                        "Open vocabulary value '{}' should be all lowercase and use hyphens instead of spaces or underscores as word separators.",
                        malware_type,
                    )));
                    continue;
                }
        ...
        if let Some(operating_system_refs) = &self.operating_system_refs {
            ...
            for operating_system_ref in operating_system_refs {
                if stix_case(&operating_system_ref.get_type()) != "software" {
                    errors.push(Error::ValidationError(format!(
        ...
        if let Some(architecture_execution_envs) = &self.architecture_execution_envs {
            ...
            for architecture_execution_env in architecture_execution_envs {
                if ArchitectureExecutionEnvs::iter()
                    .all(|x| x.as_ref() != stix_case(&architecture_execution_env.as_str()))
                {
                    errors.push(Error::ValidationError(format!(
        ...
        if let Some(implementation_languages) = &self.implementation_languages {
            ...
            for implementation_language in implementation_languages {
                if ImplementationLanguage::iter()
                    .all(|x| x.as_ref() != stix_case(&implementation_language.as_str()))
                {
                    errors.push(Error::ValidationError(format!(
...
impl Stix for MalwareAnalysis {
    ...
    fn stix_check(&self) -> Result<(), Error> {
        ...
        if *product != stix_case(&product) {
            errors.push(Error::ValidationError(
                "Product names MUST be all lowercase with words separated by a dash '-'"
                    .to_string(),
            ));
        }
        ...
        if let Some(result) = &self.result {
            if !MalwareResult::iter().any(|x| x.as_ref() == stix_case(&result)) {
                errors.push(Error::ValidationError(format!(
        ...
        if let Some(sample_ref) = &self.sample_ref {
            ...
            if !ScoTypes::iter().any(|x| x.as_ref() == stix_case(&sample_ref.get_type())) {
                errors.push(Error::ValidationError(format!(
...
impl Stix for Opinion {
    fn stix_check(&self) -> Result<(), Error> {
        ...
        if OpinionType::iter().all(|x| x.as_ref() != stix_case(&opinion)) {
            errors.push(Error::ValidationError(format!(
    ...
    fn stix_check(&self) -> Result<(), Error> {
        ...
        if let Some(report_types) = &self.report_types {
            ...
            for report_type in report_types {
                if ReportType::iter().all(|x| x.as_ref() != stix_case(&report_type)) {
                    errors.push(Error::ValidationError(format!(
                        "A report type should come from the report-type-ov open vocabulary. Report type '{}' is not in the vocabulary.",
                        report_type,
                    )));
                }
    ...
    fn stix_check(&self) -> Result<(), Error> {
        ...
        if let Some(actor_types) = &self.threat_actor_types {
            ...
            for actor_type in actor_types {
                if ThreatActorType::iter().all(|x| x.as_ref() != stix_case(&actor_type)) {
                    errors.push(Error::ValidationError(format!(
        ...
        if let Some(personal_motivations) = &self.personal_motivations {
            ...
            for personal_motivation in personal_motivations {
                if AttackMotivation::iter()
                    .all(|x| x.as_ref() != stix_case(&personal_motivation.as_str()))
                {
                    errors.push(Error::ValidationError(format!(
                        "A primary_motivation should come from the STIX pattern type open vocabulary. Threat actor primary_motivation '{}' is not in the vocabulary.",
                        personal_motivation,
    ...
    fn stix_check(&self) -> Result<(), Error> {
        ...
        if let Some(tool_types) = &self.tool_types {
            ...
            for tool_type in tool_types {
                if !ToolType::iter().any(|x| x.as_ref() == stix_case(&tool_type)) {
                    errors.push(Error::ValidationError(format!(
