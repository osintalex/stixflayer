...
impl Stix for LanguageContent {
    fn stix_check(&self) -> Result<(), Error> {
        ...
        for (key, value) in self.contents.iter() {
            ...
            // Check that subkeys are valid property names: ^[a-z0-9_]{3,250}$
            for (subkey, _) in value.iter() {
                let valid = subkey.len() >= 3
                    && subkey.len() <= 250
                    && subkey
                        .chars()
                ...
                if !valid {
                    errors.push(Error::ValidationError(format!(
                        "'{}' in '{}' of the 'contents' property is invalid and must match a valid property name",
                        subkey, key,
                    )));
                }
            }
...
impl LanguageContentBuilder {
    ...
    pub fn insert_content_objects(
        ...
        for (key, value_map) in content.iter() {
                ...
                let content_type = match prop_value {
                    ...
                    Value::Object(map) => {
                        ...
                        ContentType::Object(Box::new(
                            LanguageContentBuilder::new(&format!(
                                "{}--{}--{}",
                                object_ref,
                                key,
                                Uuid::new_v4()
            ...
            let nested_lc = LanguageContentBuilder::new(&format!(
                "{}--{}--{}",
                object_ref,
                key,
                Uuid::new_v4()
