...
impl Stix for ExtensionDefinition {
    fn stix_check(&self) -> Result<(), Error> {
        ...
        if self.description.is_none() || self.description == Some("".to_string()) {
            errors.push(Error::ValidationError(
                "Extension Definition SMOs must have a `description` property.".to_string(),
            ));
        }

        if self
            .extension_types
            .contains(&ExtensionType::ToplevelPropertyExtension)
        {
            if self.extension_properties.is_none() {
                errors.push(Error::ValidationError(
                    "When `extension_types` includes `toplevel-property-extension`, `extension_properties` must include one or more property names.".to_string(),
                ));
            }
        } else if self.extension_properties.is_some() {
            errors.push(Error::ValidationError(
                "The `extension_properties` property must only be used when `extension_types` includes a value of `toplevel-property-extension`.".to_string(),
            ));
        }
